...
const FAQ = [
  ...
  { d: 'Gli esempi mostrati funzionano sempre?', r:
`No, non sempre. Gli esempi sono testati con i modelli AI gratuiti supportati, ma il risultato dipende da molti fattori: il modello che stai usando, il suo limite di token, la sua interpretazione del prompt, e a volte semplicemente la fortuna della generazione.

Cosa fare se un esempio non produce un'app funzionante:
1. Premi 🔄 Rigenera: spesso al secondo o terzo tentativo l'output è completo
2. Prova un altro modello dalle Impostazioni (Groq è generalmente il più affidabile per JSON)
3. Modifica il prompt aggiungendo dettagli specifici su cosa ti aspetti
4. Per app complesse, dividi la richiesta in due prompt più piccoli e combinali

I modelli AI generativi non sono deterministici: la stessa richiesta può produrre risultati diversi ogni volta. Per output di qualità professionale costante, considera un modello a pagamento via OpenRouter (es. Claude Sonnet, GPT-5).` },
