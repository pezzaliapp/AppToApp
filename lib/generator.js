...
import { costruisciPromptUtente } from './templates.js';
...
export async function generaApp({ richiesta, template, config } = {}) {
  ...
  // Il template selezionato arricchisce la richiesta con istruzioni specifiche.
  const richiestaArricchita = costruisciPromptUtente(template, testo);
  const prompt =
    `Crea un'app web a partire da questa richiesta:\n"""\n${richiestaArricchita}\n"""` +
    '\n\nRispondi solo con il JSON richiesto.';

  // Diagnostica: permette di verificare che il template incida davvero sul prompt.
  console.log('[AppToApp] Prompt finale:', prompt);
