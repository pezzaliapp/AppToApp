 ...
 */
export async function chiamaLLM({ prompt, sistema, config } = {}) {
  ...
  try {
    if (cfg.provider === 'gemini') {
      return await chiamaGemini(prompt, sistema, cfg, controller.signal);
    }
    // Groq e OpenRouter condividono il formato OpenAI.
    return await chiamaOpenAICompat(prompt, sistema, cfg, controller.signal);
  } catch (errore) {
...
async function chiamaGemini(prompt, sistema, cfg, signal) {
...
async function chiamaOpenAICompat(prompt, sistema, cfg, signal) {
